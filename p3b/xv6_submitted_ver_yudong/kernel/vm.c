...
copyuvm(pde_t *pgdir, uint sz)
{
  ...
  if((d = setupkvm()) == 0)
    return 0;
  for(i = PGSIZE; i < proc->heap_bot-PGSIZE; i += PGSIZE){
    if((pte = walkpgdir(pgdir, (void*)i, 0)) == 0)
      panic("copyuvm: code pte should exist");
    if(!(*pte & PTE_P))
      panic("copyuvm: code page not present");
    pa = PTE_ADDR(*pte);
    if((mem = kalloc()) == 0)
      goto bad;
   ...
   for(i = proc->heap_bot; i < sz; i += PGSIZE){
    if((pte = walkpgdir(pgdir, (void*)i, 0)) == 0)
      panic("copyuvm: heap pte should exist");
    if(!(*pte & PTE_P))
      panic("copyuvm: heap page not present");
    pa = PTE_ADDR(*pte);
    if((mem = kalloc()) == 0)
      goto bad;
    memmove(mem, (char*)pa, PGSIZE);
   ...
   for(i = proc->stack_low_bound; i < USERTOP; i += PGSIZE){
    if((pte = walkpgdir(pgdir, (void*)i, 0)) == 0)
